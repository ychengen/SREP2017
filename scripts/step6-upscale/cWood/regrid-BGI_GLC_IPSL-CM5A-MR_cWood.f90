!!!=================================================================
!!! Regrid 1km to IPSL-CM5A-MR resolution for 22 vegetation types - cWood
!!!=================================================================
...
character(len=*), parameter :: FNAME1 =     &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/Observation/BGI2014/1km/BGI_cWood_1km.nc"
...
character(len=*), parameter :: FNAME4 =     &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/CMIP5/IPSL-CM5A-MR/1982-2005/cWood_Lmon_IPSL-CM5A-MR_historical_r1i1p1_198201-200512.nc"
character(len=*), parameter :: FOUTNAME =   &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/CMIP5/landcover_GLC2000_for_CMIP5/cWood/landcover_BGI_cWood_IPSL-CM5A-MR.nc"
...
!! For forest type only sum cWood = forest_total(i,j)
status = nf90_def_var(nc_id3, VOUT8_NAME, NF90_DOUBLE, dimid1, var_id10)
if(status /= nf90_NoErr) call handle_err(status)

!! For forest type only area cWood = forest_area(i,j)
status = nf90_def_var(nc_id3, VOUT9_NAME, NF90_DOUBLE, dimid1, var_id11)
if(status /= nf90_NoErr) call handle_err(status)

!! For forest type only mean cWood, per forest area = forest_mean(i,j)
status = nf90_def_var(nc_id3, VOUT10_NAME, NF90_DOUBLE, dimid1, var_id12)
if(status /= nf90_NoErr) call handle_err(status)

!! For forest type only mean cWood, per cell area = grid_mean(i,j)
