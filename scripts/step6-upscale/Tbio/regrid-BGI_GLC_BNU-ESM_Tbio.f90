!!!=================================================================
!!! Regrid 1km to BNU-ESM resolution for 22 vegetation types - Tbio
!!!=================================================================
...
character(len=*), parameter :: FNAME1 =     &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/Observation/BGI2014/1km/BGI_Tbio_1km.nc"
...
character(len=*), parameter :: FNAME4 =     &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/CMIP5/BNU-ESM/1982-2005/cVeg_Lmon_BNU-ESM_historical_r1i1p1_198201-200512.nc"
character(len=*), parameter :: FOUTNAME =   &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/CMIP5/landcover_GLC2000_for_CMIP5/Tbio/landcover_BGI_Tbio_BNU-ESM.nc"
...
!write (*,*) i,"=",lon_in4(i),",",j,"=",lat_in4(j),",",x,"=",lon_in(x),",",y,"=",lat_in(y)
       ...
       IF (i.eq.1 .and. lon_in(x).ge.lonbnd_in(2,NLONS2)) THEN   !! add toward the first lon grid
        ...
        IF ( varin1(x,y)>0.0 .and. varin1(x,y)<1.e+20 ) THEN
...
!!!! For forest types (1-10) only
       ...
       ELSEIF (lon_in(x).ge.lonbnd_in(1,i) .and. lon_in(x).lt.lonbnd_in(2,i) ) THEN
        IF(varin0(x,y).ge.1.e10) then
          print *, y,x,varin0(x,y)
        ENDIF
...
!! For forest type only sum Tbio = forest_total(i,j)
status = nf90_def_var(nc_id3, VOUT8_NAME, NF90_DOUBLE, dimid1, var_id10)
if(status /= nf90_NoErr) call handle_err(status)

!! For forest type only area Tbio = forest_area(i,j)
status = nf90_def_var(nc_id3, VOUT9_NAME, NF90_DOUBLE, dimid1, var_id11)
if(status /= nf90_NoErr) call handle_err(status)

!! For forest type only mean Tbio, per forest area = forest_mean(i,j)
status = nf90_def_var(nc_id3, VOUT10_NAME, NF90_DOUBLE, dimid1, var_id12)
if(status /= nf90_NoErr) call handle_err(status)

!! For forest type only mean Tbio, per cell area = grid_mean(i,j)
