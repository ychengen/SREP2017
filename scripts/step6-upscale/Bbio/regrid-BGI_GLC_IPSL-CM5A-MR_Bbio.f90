!!!=================================================================
!!! Regrid 1km to IPSL-CM5A-MR resolution for 22 vegetation types - Bbio
!!!=================================================================

PROGRAM REGRID_1KM_HALF_DEG
...
character(len=*), parameter :: FNAME1 =     &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/Observation/BGI2014/1km/BGI_Bbio_1km.nc"
character(len=*), parameter :: FNAME2 =     &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/Observation/GLC2000/GLC2000_BGI.nc"
character(len=*), parameter :: FNAME4 =     &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/CMIP5/IPSL-CM5A-MR/1982-2005/cRoot_Lmon_IPSL-CM5A-MR_historical_r1i1p1_198201-200512.nc"
character(len=*), parameter :: FOUTNAME =   &
  "/lustre/atlas/scratch/ceyang1/cli017/BGC-Biomass/CMIP5/landcover_GLC2000_for_CMIP5/Bbio/landcover_BGI_Bbio_IPSL-CM5A-MR.nc"

integer, parameter :: NDIMS=3, NTYPES=22, NLATS1=5000, NLONS1=36000, &
                      NLATS2=143, NLONS2=144, NTIME2=288
...
if(status /= nf90_NoErr) call handle_err(status)
status = nf90_inq_varid(nc_id1, VARIN1_NAME, var_id1)
if(status /= nf90_NoErr) call handle_err(status)
...
!!! Define new 64x128 lat/lon and 22 vegetation types
...
!      DO x=50*i+1,50*i+50
      DO x=1,NLONS1
        IF (lon_in(x).lt.0.) THEN
          lon_in(x) = 360.0+lon_in(x)    !! convert -360.0+180 to 0-360
        ENDIF
...
!! For forest type only sum Bbio = forest_total(i,j)
status = nf90_def_var(nc_id3, VOUT8_NAME, NF90_DOUBLE, dimid1, var_id10)
if(status /= nf90_NoErr) call handle_err(status)

!! For forest type only area Bbio = forest_area(i,j)
status = nf90_def_var(nc_id3, VOUT9_NAME, NF90_DOUBLE, dimid1, var_id11)
if(status /= nf90_NoErr) call handle_err(status)

!! For forest type only mean Bbio, per forest area = forest_mean(i,j)
status = nf90_def_var(nc_id3, VOUT10_NAME, NF90_DOUBLE, dimid1, var_id12)
if(status /= nf90_NoErr) call handle_err(status)

!! For forest type only mean Bbio, per cell area = grid_mean(i,j)
